...
import { NBOClient } from '@/lib/nbo-logic';
import { generateRandomNBOClients } from '@/lib/nbo-random';
...
const NBOUpload = () => {
  ...
  const [generatedData, setGeneratedData] = useState<NBOClient[] | null>(null);
  ...
  const saveAndNavigate = useCallback(async (data: NBOClient[], fileName = 'Upload') => {
    const { data: session } = await supabase.auth.getSession();
    if (session?.session?.user) {
      await supabase.from('nbo_uploads').insert({
    ...
    localStorage.setItem('nbo_data_uploaded', 'true');
    navigate('/nbo/dashboard', { state: { clientData: data } });
  }, [navigate]);
  ...
  const loadFromHistory = useCallback(async (id: string) => {
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data?.client_data) {
      navigate('/nbo/dashboard', { state: { clientData: data.client_data as unknown as NBOClient[] } });
    }
  }, [navigate]);

  const deleteUpload = useCallback(async (id: string) => {
    await supabase.from('nbo_uploads').delete().eq('id', id);
    setHistory(prev => prev.filter(h => h.id !== id));
  }, []);

  const processData = useCallback((data: NBOClient[], fileName: string) => {
    ...
    if (ext === 'csv') {
      ...
      reader.onload = (e) => {
        try {
          ...
          processData(mapColumns(rows), file.name);
  ...
  const mapColumns = (rows: Record<string, any>[]): NBOClient[] => {
    ...
    const gastoCol = findCol(sample, ['gasto', 'valor', 'total', 'revenue', 'spend']);
    const catCol = findCol(sample, ['categoria', 'category', 'segmento']);
    const diasCol = findCol(sample, ['ultima_compra', 'dias', 'recencia', 'last_purchase']);
    if (!gastoCol) throw new Error('Coluna de gasto/valor não encontrada. Verifique se o arquivo contém: gasto_total ou valor.');
    return rows.map((r, i) => ({
      nome: r[nomeCol || ''] || `Cliente ${i + 1}`,
      id_cliente: String(r[idCol || ''] || i + 1),
      gasto_total: Number(r[gastoCol]) || 0,
      categoria_preferida: r[catCol || ''] || 'N/A',
      ultima_compra_dias: Number(r[diasCol || ''] || 0),
    }));
  };
  ...
  const handleGenerate = () => setGeneratedData(generateRandomNBOClients(clientCount));

  const handleDownloadExcel = () => {
    if (!generatedData) return;
    const ws = XLSX.utils.json_to_sheet(generatedData.map(c => ({
      Nome: c.nome, ID_Cliente: c.id_cliente, Gasto_Total: c.gasto_total,
      Categoria_Preferida: c.categoria_preferida, Ultima_Compra_Dias: c.ultima_compra_dias,
    })));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Clientes');
    XLSX.writeFile(wb, `clientes_nbo_${clientCount}.xlsx`);
  };
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">Carregue sua base de clientes para a análise Next Best Offer</p>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              A planilha deve conter: <strong>Nome, ID do Cliente, Gasto Total</strong> (valor acumulado),
              <strong> Categoria Preferida</strong> (opcional), <strong>Última Compra</strong> (dias).
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card className="mb-6 p-6">
        ...
        <div className="mb-4">
          ...
          <Slider value={[clientCount]} onValueChange={(v) => setClientCount(v[0])} min={50} max={200} step={10} className="my-3" />
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {generatedData && (
            <>
              ...
              <Button onClick={() => saveAndNavigate(generatedData, `Dados aleatórios (${generatedData.length} clientes)`)}>Usar para Análise →</Button>
            </>
          )}
        </div>
        {generatedData && <p className="mt-3 text-xs text-muted-foreground">✅ {generatedData.length} clientes gerados com sucesso.</p>}
      </Card>
              ...
              <TableBody>
                {history.map((h) => (
                  <TableRow key={h.id}>
                    ...
                    <TableCell className="text-center"><Badge variant="secondary">{h.client_count}</Badge></TableCell>
